...
/* Includes ------------------------------------------------------------------*/
#include "usbpd_tcpp0203_usbpd_pwr.h"
#include "usbpd_tcpp0203_bus.h"
...
int32_t BSP_USBPD_PWR_Init(uint32_t PortNum)
{
  ...
  {
    #if defined (_TRACE)||(_TRACE2)
      BSP_USBPD_PWR_TRACE(PortNum, "-- BSP_USBPD_PWR_Init --");
    #elif (_TRACE3)
      printf("-- BSP_USBPD_PWR_Init --");
    #endif
  ...
  else
  {
    switch (Role)
    {
      case POWER_ROLE_SOURCE:
        ...
        #elif (_TRACE3)
          printf("-- BSP_USBPD_PWR_SetRole : SRC --");
        #endif
        /* Set port power Role */
      ...
      case POWER_ROLE_SINK:
        ...
        #elif (_TRACE3)
          printf("-- BSP_USBPD_PWR_SetRole : SNK --");
        #endif
        /* Set port power Role */
        USBPD_PWR_Port_Status[PortNum].PwrRole = Role;
        break;
...
int32_t BSP_USBPD_PWR_SetPowerMode(uint32_t PortNum, USBPD_PWR_PowerModeTypeDef PwrMode)
{
  ...
  else
  {
    ...
    #elif (_TRACE3)
      printf("-- BSP_USBPD_PWR_SetPowerMode --");
    #endif
    ...
    switch (PwrMode)
    {
      ...
      case USBPD_PWR_MODE_LOWPOWER:
        /* Port Mode is Low Power : suitable while toggling SNK/SRC is case of DRP
             - Dead Batteries Enabled
             - Gate Driver Consumer controlled by I2C, Gate Driver Provider controlled by I2C
             - Toggling on both CCs
        */
        #if defined (_TRACE)||(_TRACE2)
          BSP_USBPD_PWR_TRACE(PortNum, "-- Low Power --");
        #elif (_TRACE3)
      ...
      case USBPD_PWR_MODE_NORMAL:
        /* Port Mode is Normal : suitable for SRC config
             - Dead Batteries Disabled
             - Gate Driver Consumer controlled by I2C, Gate Driver Provider enabled (VCONN, VSW, etc.)
        */
        #if defined (_TRACE)||(_TRACE2)
...
int32_t BSP_USBPD_PWR_GetPowerMode(uint32_t PortNum, USBPD_PWR_PowerModeTypeDef *PwrMode)
{
  ...
  else
  {
    ...
    #elif (_TRACE3)
      printf("-- BSP_USBPD_PWR_GetPowerMode --");
    #endif
...
int32_t BSP_USBPD_PWR_VBUSDeInit(uint32_t PortNum)
{
  ...
  else
  {
    ...
    #elif (_TRACE3)
      printf("-- BSP_USBPD_PWR_VBUSDeInit --");
    #endif
        ...
        (USBPD_PWR_HW_CONFIG_TYPE_TCPP03 == USBPD_PWR_Port_Configs[PortNum].Type))
    {
      ...
      #elif (_TRACE3)
        printf("-- GDP/GDC setting : default --");
      #endif
